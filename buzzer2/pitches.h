...
void music_play(int melody[], int noteDurations[], int length, int pin, int time_constant){
  for (int thisNote = 0; thisNote < length; thisNote++) {
 
    // to calculate the note duration, take one second divided by the note type.
    //e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
    int noteDuration = time_constant/noteDurations[thisNote];
...
void music_start(int pin){
  //notes in the melody
//notes in the melody:
int melody[] = { NOTE_E3,NOTE_E3,NOTE_00,NOTE_E3,NOTE_00,NOTE_C3,NOTE_E3, NOTE_00,NOTE_G3, NOTE_00, NOTE_00, NOTE_G2, NOTE_00, NOTE_00}; 
// note durations: 4 = quarter note, 8 = eighth note, etc.: 
int noteDurations[] = { 8, 8, 8, 8, 8, 8, 8, 8, 8, 4, 8, 8, 8, 4}; 
  int length = (sizeof(melody) / sizeof(melody[0]));
  
  music_play(melody, noteDurations, length, pin, 1000);
}

void music_end(int pin){
  //notes in the melody
  int melody[] = {
  ...
  music_play(melody, noteDurations, length, pin, 1800);
}

void music_frog(int pin){
  //notes in the melody
  int melody[] = {
  ...
  music_play(melody, noteDurations, length, pin, 1600);
}

void music_mario(int pin) {
	//notes in the melody
	int melody[] = {
